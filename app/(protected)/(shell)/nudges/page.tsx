...
import { DeliveryRulesCard } from "@/components/nudges/delivery-rules-card";
...
export default async function NudgesPage({ searchParams }: { searchParams: Promise<{ filter?: string | string[] }> }) {
  ...
  const [nudges, appSettings] = await Promise.all([getNudges(filter), getAppSettings()]);
    ...
    <>
      ...
      <div className={pageStyles.content}>
        ...
        <div className={styles.layout}>
          ...
          <DeliveryRulesCard
            initialSettings={{
              morningBrief: appSettings.morningBrief,
              eveningCheckIn: appSettings.eveningCheckIn,
              streakWarnings: appSettings.streakWarnings,
              weeklyReviewPrompt: appSettings.weeklyReviewPrompt,
            }}
          />
        </div>
      </div>
