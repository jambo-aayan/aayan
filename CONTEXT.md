...
One block of a Pillar or Area page — North Star, Goals, Habits, Systems, Tasks, Thoughts, Charts, or Table (the last two added by ADR-0017 — see "Visual" below), each scoped to that Pillar/Area. A Pillar/Area page is a **reorderable, toggleable list** of these, not a fixed layout — the user picks which show and in what order, per Pillar/Area (a systems-heavy Pillar can feature Systems and hide Tasks, say). Deliberately generic groundwork for a much larger "modular, Notion-like" ambition (see "Future direction" below) without building that ambition yet. Charts and Table are the one exception to "each section shows at most once": they're **zones**, each holding any number of Visual instances, not a single card — see "Visual". A page can also carry fixed, non-section content above its section list for content that doesn't fit the generic model (e.g. Health's Ankylosing Spondylitis Area keeps its bespoke Pain & Mobility Tracking card this way); Finances' entire page is exactly this kind of fixed content, with no section list today.

**Visual** (ADR-0017):
One chart or table instance placed in a Pillar/Area page's Charts or Table zone (see "Page section") — a Line, Bar, Progress bar, Scatter, or Streak heatmap chart, or a freeform/bound Table. A page can hold any number of Visuals in each zone. A Visual's data is either **ad-hoc** (entered directly — one record at a time, defaulting to today's date, or in bulk via pasted text or a CSV upload) or **bound** to an existing domain source (a Habit's check-ins, a System's evaluation scores, a Goal's progress, a Finance Account's balances/transactions for charts; Goals/Habits/Tasks/Systems for a Table) via a live **adapter** — resolved fresh at render time, never a stale copy. A bound Table's rows are the real entities themselves, showing the adapter's built-in read-only columns (e.g. Name, Status) plus any custom columns the user adds on top, keyed to that row's real entity. Scatter is the one chart type with two independent bindings (X and Y each separately ad-hoc or bound), enabling correlation-style charts. A "Log today" quick-add for ad-hoc, date-based charts surfaces on Home, alongside the Daily Focus habits widget, so logging a metric doesn't require opening its Pillar/Area page first.
_Avoid_: Don't confuse a bound Visual with a copy/sync of its source data — there is no duplicate, no sync job; it's read live every time.
