...
- **Account linking** (MVP): bank accounts, credit cards, and investment accounts connect via API for automatic balance/transaction sync, rather than manual entry only. **Provider not yet chosen** — see [wayfinder ticket #5](https://github.com/jambo-aayan/aayan/issues/5), researching a genuinely free (or near-free) option given ADR-0001's "free to run" constraint, which most bank-aggregation APIs don't meet at real usage levels. Investment account coverage is expected to be weaker than bank/credit card coverage (UK Open Banking regulation mainly covers payment accounts) — manual entry likely stays the fallback for investments regardless of what's chosen for banking. Linked accounts still populate as **Items** and **Transactions** — this is a data-source decision, not a new domain concept.
