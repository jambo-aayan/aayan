...
- **Account linking** (MVP): bank accounts, credit cards, and (for this user specifically) the investment account all connect via API for automatic balance/transaction/position sync, rather than manual entry only.
  - **Bank + credit card**: via an Open Banking aggregator. **Enable Banking's "Restricted Production" tier is the lead candidate** (research: [docs/research/0001-account-linking-apis.md](https://github.com/jambo-aayan/aayan/blob/research/finance-api-linking/docs/research/0001-account-linking-apis.md) on the `research/finance-api-linking` branch) — its terms weren't independently verified due to a network block during research, so confirm directly before implementation. GoCardless (the historically free option) is closed to new signups since July 2025 — ruled out. **Plaid checked and ruled out**: UK/EU production access requires a sales conversation (no self-serve pricing), and pay-as-you-go isn't available in Europe at all — doesn't fit the free-to-run constraint regardless of bank coverage.
  ...
  - **The user's actual accounts**: bank is **Lloyds** (CMA9-mandated — near-guaranteed aggregator coverage), credit card is **Yonder** (newer, smaller issuer, not CMA9-mandated — read-only coverage under Enable Banking **not confirmed**, check at implementation time, manual entry as fallback), investment is **Trading 212** S&S ISA (own API, see above).
